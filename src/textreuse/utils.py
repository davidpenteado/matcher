...
def strip_html_markup(text: str) -> str:
    """Remove simple HTML/XML markup while preserving readable text boundaries."""
    if not text:
        return ""

    # Avoid touching ordinary OCR text unless it plausibly contains tags.
    if not re.search(r'<\s*/?\s*[a-zA-Z][^>]*>', text):
        return text

    text = re.sub(r'<!--.*?-->', ' ', text, flags=re.DOTALL)
    text = re.sub(r'<\s*(script|style)\b[^>]*>.*?<\s*/\s*\1\s*>', ' ', text, flags=re.DOTALL | re.IGNORECASE)
    text = re.sub(r'<\s*br\s*/?\s*>', '\n', text, flags=re.IGNORECASE)
    text = re.sub(r'<\s*/?\s*(article|section|div|p|h[1-6]|li|ul|ol|blockquote|table|tr|td|th)\b[^>]*>', '\n', text, flags=re.IGNORECASE)
    text = re.sub(r'<[^>]+>', ' ', text)
