...
def run(pt_dir, corpus, out_dir, index_dir, min_score, include_corpora, viz):
    ...
    if include_corpora:
        ...
        # Expand aliases like "Medical"
        include_list = MultiLanguageIndexer.expand_corpora(include_list)
        
        # Filter to only those that actually have built indices
        available_corpora = []
        for c in include_list:
            if c in MultiLanguageIndexer.LANGUAGE_INDEX_MAP:
                idx_dir = Path(index_dir) / MultiLanguageIndexer.LANGUAGE_INDEX_MAP[c]
                if (idx_dir / "index.faiss").exists():
                    available_corpora.append(c)
        include_list = available_corpora
