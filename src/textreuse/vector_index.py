...
class VectorIndexer:
    ...
    def build_index(self, corpus_path: str, append: bool = False, corpus_prefix: str = None):
        ...
        if not append:
            self.clear()
            self._init_db()
            self.index = faiss.IndexScalarQuantizer(self.dimension, faiss.ScalarQuantizer.QT_8bit, faiss.METRIC_INNER_PRODUCT)
            start_key = 0
        else:
            if not self.load():
        ...
        # We need to buffer embeddings if the index is untrained
        training_buffer_vecs = []
        training_buffer_rows = []
        
        def flush_batch(force_train=False):
            nonlocal processed_count
            if not current_chunks and not training_buffer_vecs:
                return
            ...
            if current_chunks:
                ...
                if not self.index.is_trained:
                    training_buffer_vecs.append(embeddings)
                    training_buffer_rows.extend(current_db_rows)
                    
                    total_buffered = sum(len(b) for b in training_buffer_vecs)
                    # Train if we hit 50k vectors, or if this is the final flush_batch (force_train)
                    if total_buffered >= 50000 or force_train:
                        logger.info(f"Training ScalarQuantizer on {total_buffered} vectors...")
                        train_data = np.vstack(training_buffer_vecs)
                        self.index.train(train_data)
                        logger.info("Training complete. Adding buffered vectors...")
                        self.index.add(train_data)
                        
                        conn = sqlite3.connect(self.db_path)
                        c = conn.cursor()
                        c.executemany("INSERT OR REPLACE INTO metadata VALUES (?, ?, ?, ?, ?, ?)", training_buffer_rows)
                        conn.commit()
                        conn.close()
                        
                        processed_count += len(training_buffer_rows)
                        training_buffer_vecs.clear()
                        training_buffer_rows.clear()
            ...
            # Handle straggler vectors that were buffered if force_train is called and we never reached 50k
            if force_train and not self.index.is_trained and training_buffer_vecs:
                logger.info(f"Finalizing ScalarQuantizer training on {sum(len(b) for b in training_buffer_vecs)} vectors...")
                train_data = np.vstack(training_buffer_vecs)
                self.index.train(train_data)
                self.index.add(train_data)
        ...
        for doc in Ingestor.iter_corpus(corpus_path):
            ...
            for idx, (chunk_text, start, end) in enumerate(doc_chunks):
                ...
                key = start_key + processed_count + len(current_chunks) - 1 + len(training_buffer_rows)
                current_db_rows.append((key, doc_id, doc.get('title', ''), start, end, chunk_text))
                
                if len(current_chunks) >= 100000:
                    flush_batch(force_train=False)
                    self.save() 
        
        flush_batch(force_train=True)
...
class MultiLanguageIndexer:
    LANGUAGE_INDEX_MAP = {
        ...
        "Medical_English": "index_medical_english", "Medical_French": "index_medical_french",
        "Medical_German": "index_medical_german", "Medical_Italian": "index_medical_italian",
        "Medical_Spanish": "index_medical_spanish"
    }
    
    # Aliases: a single name that expands to multiple corpora
    CORPUS_ALIASES = {
        "Medical": ["Medical_English", "Medical_French", "Medical_German", "Medical_Italian", "Medical_Spanish"]
    }
    ...
    @classmethod
    def expand_corpora(cls, corpora_list: list) -> list:
        """Expand any aliases in the corpora list."""
        if not corpora_list:
            return corpora_list
        expanded = []
        for c in corpora_list:
            if c in cls.CORPUS_ALIASES:
                expanded.extend(cls.CORPUS_ALIASES[c])
            else:
                expanded.append(c)
        return expanded
    ...
    def _get_index(self, language: str):
        ...
        if language not in self._indices:
            ...
            # Use memory-mapped I/O to avoid MemoryError on large indices
            index = faiss.read_index(str(faiss_path), faiss.IO_FLAG_MMAP)
            logger.info(f"Loaded {language} index: {index.ntotal:,} vectors (mmap)")
